...
def roi_to_40x_bbox(roi: dict) -> list:
    """Convert a visited_roi dict to [x1, y1, x2, y2] in 40x pixel coordinates.

    x, y are centre coordinates at the base (40x) level.
    size is the patch side length in pixels at the given magnification.
    Scaling to 40x: size_40x = size * (40 / mag).
    """
    x, y = roi["x"], roi["y"]
    size_40x = roi["size"] * (40.0 / roi["magnification"])
    half = size_40x / 2.0
...
def evaluate_case(json_path: str, gt_index: dict[str, str],
    ...
    """Evaluate one case from a visited_rois JSON file. Returns None if no GT."""
    base = os.path.splitext(os.path.basename(json_path))[0]
    # Strip the last _Suffix (e.g. _Subtype) to recover the bare case ID
    case_name = base.rsplit("_", 1)[0]
    ...
    roi_data = {}
    roi_metas = {}
    for i, roi in enumerate(visited_rois, start=1):
        roi_name = f"roi_{i}"
        roi_data[roi_name] = roi_to_40x_bbox(roi)
        mag = float(roi.get("magnification", 40.0))
        size_40x = float(roi.get("size", 0)) * (40.0 / mag)
        roi_metas[roi_name] = {"mag": mag, "roi_w": size_40x, "roi_h": size_40x}
...
def main():
    ...
    # Collect all visited_rois JSON files across all result folders; skip *_lesions.json
    json_files = []
    for result_folder in args.result_folders:
        if not os.path.isdir(result_folder):
            print(f"Warning: result folder not found, skipping: {result_folder}")
            continue
        for fn in sorted(os.listdir(result_folder)):
            if fn.endswith(".json") and not fn.endswith("_lesions.json"):
                json_files.append(os.path.join(result_folder, fn))
    ...
    for json_path in json_files:
        res = evaluate_case(json_path, gt_index, qa_index, hit_threshold_cache)
        if res is None:
            skipped_no_gt += 1
            continue
