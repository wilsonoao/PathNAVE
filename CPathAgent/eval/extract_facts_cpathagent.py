"""
Extract atomic pathology facts from CPathAgent output JSON files.

For each output JSON:
  - stage1 descriptions: get IDs from regions[*].group.id_list,
    look up process.stage1_screening.descriptions[str(id)]
  - stage3 patch descriptions: process.regions[*].stage3_reasoning.patch_descriptions
    (deduplicated across regions)
  - Each description text is sent to LLM with the fact-extraction prompt
...
Usage:
  ...
  python extract_facts_cpathagent.py --provider openai --model gpt-4o \
    --input_dir ../output/test_tcga_luad/TCGA_LUAD \
    --output_dir result_facts_tcga_luad
"""
...
def load_cpathagent_results(input_dir: Path):
    """Yield (json_path, list[dict]) for every CPathAgent output JSON found.

    Each dict has keys: source, source_text, and source-specific fields.
    Sources:
      - "stage1_description": id field
      - "stage3_patch_description": region_index, patch_index fields
    """
    for json_file in sorted(input_dir.rglob("*.json")):
        # Skip temporary image directories
        if "_tmp_images" in json_file.parts:
            continue
        ...
        s1_descs = proc.get("stage1_screening", {}).get("descriptions", {})

        texts = []
        seen_ids: set = set()
        seen_patch_texts: set = set()

        for region in regions:
            id_list = region.get("group", {}).get("id_list", [])

            # stage1 descriptions keyed by str(id)
            for pid in id_list:
                if pid in seen_ids:
                    continue
                seen_ids.add(pid)
                desc_text = s1_descs.get(str(pid))
                if desc_text:
                    texts.append({
                        "source": "stage1_description",
                        "id": pid,
                        "text": desc_text,
                    })

            # stage3 patch descriptions (deduplicated by content)
            region_idx = region.get("region_index", 0)
            patch_descs = region.get("stage3_reasoning", {}).get("patch_descriptions", [])
            for patch_idx, pd_text in enumerate(patch_descs):
                if not pd_text or pd_text in seen_patch_texts:
                    continue
                seen_patch_texts.add(pd_text)
                texts.append({
                    "source": "stage3_patch_description",
...
def main():
    parser = argparse.ArgumentParser(description="Extract atomic pathology facts from CPathAgent outputs")
    parser.add_argument("--provider", choices=["openai", "ollama"], required=True)
    parser.add_argument("--model", required=True,
                        help="e.g. gpt-4o (OpenAI) or llama3 (Ollama)")
    parser.add_argument("--input_dir", required=True,
                        help="Directory containing CPathAgent output JSON files (searched recursively)")
    ...
    for json_file, texts in load_cpathagent_results(input_dir):
        total_files += 1
        ...
        for entry in texts:
            if entry["source"] == "stage1_description":
                label = f"stage1 id={entry['id']}"
            else:
                label = f"stage3 region={entry['region_index']} patch={entry['patch_index']}"
            ...
            if entry["source"] == "stage1_description":
                ...
                out_entry["region_index"] = entry["region_index"]
                out_entry["patch_index"] = entry["patch_index"]
