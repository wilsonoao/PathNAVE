...
def load_all_vqa_pairs(vqa_file, dataset_name='wsi_vqa', image_dir=None):
    ...
    # ---------- 2. SlideBench-VQA (TCGA) ----------
    ...
    elif dataset_name.lower() == 'camelyon16':
        with open(vqa_file, "r", encoding="utf-8") as f:
            data = json.load(f)
        ...
        if image_dir is not None:
            dirs = [d for d in os.listdir(image_dir)]
        else:
            dirs = case2qas.keys()
    ...
    elif dataset_name.lower() == 'tcga_brca':
        with open(vqa_file, "r", encoding="utf-8") as f:
            data = json.load(f)
        ...
        for d in dirs:
            ...
            if long_id not in case2qas:
                continue

            for qa in case2qas[long_id]:
                vqa_pairs.append({
                    "long_id": long_id,
                    "question": qa["Question"],
                    "choices": None,
