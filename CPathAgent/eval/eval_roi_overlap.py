...
Input JSON format: CPathAgent output JSON files (one per case).
  ROIs are extracted from process.regions[*].stage2_navigation.steps using
  region_coordinate + magnification to recover the full 40x pixel bounding box.
  When regions is empty, a whole-slide bounding box is auto-sized from
  groups_all total_patches * patch_size (ceil-square grid approximation).
...
def _is_normalized(coord: list) -> bool:
    """Return True if all coordinate values are in [0, 1] (WSI-normalised space)."""
    return all(0.0 <= v <= 1.0 for v in coord)


def _auto_wsi_size(data: dict, patch_size: int) -> tuple[int, int] | None:
    """Estimate WSI dimensions from the total patch count in stage1 groups_all.

    Treats all patches as a square grid: cols = ceil(sqrt(N)), rows = ceil(N/cols).
    Returns (W_px, H_px) where each dimension = grid_cols/rows * patch_size.
    This is a rough approximation used when exact slide dimensions are unavailable.
    """
    groups_all = (data.get("process", {})
                      .get("stage1_screening", {})
                      .get("groups_all", []))
    total = sum(len(g.get("id_list", [])) for g in groups_all)
    if total == 0:
        return None
    cols = math.ceil(math.sqrt(total))
    rows = math.ceil(total / cols)
    return (cols * patch_size, rows * patch_size)


def load_id_bbox_map(bbox_dir: str) -> dict[int, list]:
    """Load patch_id -> bbox mapping from all JSON files in bbox_dir.

    Each JSON file has string patch_id keys and [x1, y1, x2, y2] bbox values
    at the highest magnification coordinate space.
    """
    id_bbox_map: dict[int, list] = {}
    for fn in sorted(glob.glob(os.path.join(bbox_dir, "*.json"))):
        try:
            with open(fn) as f:
                data = json.load(f)
        except Exception as e:
            print(f"[bbox map load error] {fn}: {e}")
            continue
...
def collect_ids_with_bboxes(
    json_path: str, id_bbox_map: dict[int, list]
) -> list[dict]:
    """Return all patch IDs from every region, each paired with its highest-magnification bbox.

    Iterates process.regions and looks up each id_list entry in id_bbox_map.
    Entries whose ID is not found in the map have bbox=None.
    """
    with open(json_path) as f:
        data = json.load(f)

    results = []
    for region in data.get("process", {}).get("regions", []):
        region_index = region.get("region_index")
        group_name = region.get("group", {}).get("name", "")
        for patch_id in region.get("group", {}).get("id_list", []):
            results.append({
                "region_index": region_index,
                "group_name": group_name,
                "id": patch_id,
                "bbox": id_bbox_map.get(patch_id),
            })
    return results


def extract_rois_from_agent_json(
    json_path: str,
    patch_size: int,
    wsi_size: tuple[int, int] | None = None,
) -> tuple[dict, int]:
    """Extract ROI bounding boxes from a CPathAgent output JSON.

    Iterates process.regions; for each region reads stage2_navigation.steps and
    reconstructs the full 40x-pixel subpatch bounding box by expanding the stored
    zoomed crop (region_coordinate) by its magnification factor.

    When process.regions is empty (agent stayed at screening level), a single
    whole-slide bounding box is returned, sized from groups_all patch count.

    Returns
    -------
    roi_data : dict mapping roi_name -> [x1, y1, x2, y2] in 40x pixels.
    total_patches : total stage1 patches used across all regions' groups.
    """
    with open(json_path) as f:
        data = json.load(f)

    proc = data.get("process", {})
    regions = proc.get("regions", [])

    total_patches = sum(
        len(r.get("group", {}).get("id_list", []))
        for r in regions
    )

    if not regions:
        # Whole slide: approximate dimensions from groups_all total patches
        effective_size = wsi_size or _auto_wsi_size(data, patch_size)
        if effective_size is None:
            return {}, 0, {}
        W, H = effective_size
        return (
            {"whole_slide": [0.0, 0.0, float(W), float(H)]},
            0,
            {"whole_slide": {"mag": 1.0, "roi_w": float(W), "roi_h": float(H)}},
        )

    # Resolve wsi_size once for the entire file (used only for normalised coords)
    effective_wsi_size = wsi_size or _auto_wsi_size(data, patch_size)

    rois: dict[str, list] = {}
    roi_metas: dict[str, dict] = {}
    for r_idx, region in enumerate(regions):
        nav = region.get("stage2_navigation") or {}
        steps = nav.get("steps", [])

        for step in steps:
            mag = float(step.get("magnification", 1.0))
            coord = step.get("region_coordinate")
            ...
            if _is_normalized([x1, y1, x2, y2]):
                if effective_wsi_size is None:
                    continue
                W, H = effective_wsi_size
                # Expand normalised zoomed crop → full subpatch in 40x pixels
                cx = (x1 + x2) / 2.0 * W
                cy = (y1 + y2) / 2.0 * H
                half_w = (x2 - x1) * mag / 2.0 * W
                half_h = (y2 - y1) * mag / 2.0 * H
            else:
                # Absolute 40x-pixel coords: expand zoomed crop back to full subpatch
                cx = (x1 + x2) / 2.0
                cy = (y1 + y2) / 2.0
                half_w = (x2 - x1) * mag / 2.0
                half_h = (y2 - y1) * mag / 2.0

            roi_name = f"region_{r_idx}_step_{step.get('step', len(rois) + 1)}"
            rois[roi_name] = [cx - half_w, cy - half_h, cx + half_w, cy + half_h]
            roi_metas[roi_name] = {"mag": mag, "roi_w": half_w * 2, "roi_h": half_h * 2}

    return rois, total_patches, roi_metas


def evaluate_case(json_path: str, gt_index: dict[str, str],
                  patch_size: int,
                  wsi_size: tuple[int, int] | None = None,
                  qa_index: dict[str, bool | None] | None = None,
                  id_bbox_map: dict[int, list] | None = None,
                  hit_threshold_cache: dict | None = None) -> dict | None:
    """Evaluate one case from a CPathAgent output JSON.

    Returns None if no matching GT or if regions is empty (whole-slide implied).
    When id_bbox_map is provided, each patch ID in every region's id_list is
    looked up and its highest-magnification bbox is added as an additional ROI
    named region_{r_idx}_id_{patch_id}.
    """
    with open(json_path) as f:
        meta = json.load(f)
    case_name = meta.get("case_id",
    ...
    roi_data, total_patches, roi_metas = extract_rois_from_agent_json(json_path, patch_size, wsi_size)

    if id_bbox_map:
        for region in meta.get("process", {}).get("regions", []):
            r_idx = region.get("region_index", 0)
            for patch_id in region.get("group", {}).get("id_list", []):
                bbox = id_bbox_map.get(patch_id)
                if bbox and len(bbox) == 4:
                    rn = f"region_{r_idx}_id_{patch_id}"
                    roi_data[rn] = list(map(float, bbox))
                    roi_metas[rn] = {
                        "mag": 40.0,
                        "roi_w": abs(bbox[2] - bbox[0]),
                        "roi_h": abs(bbox[3] - bbox[1]),
                    }

    if not roi_data:
        return None  # no ROI could be extracted
    ...
    # Priority: explicit wsi_size arg > CSV wsi_40x_width/height > sqrt(total_size) > GT bounds.
    # _auto_wsi_size is intentionally NOT used here: it estimates slide size from groups_all
    # patch counts which often represent only a subset of the full slide, leading to a grossly
    # underestimated slide area and sample_x_max < 0 (early-return 0.0) for any large ROI.
    slide_w = float(wsi_size[0]) if wsi_size else csv_wsi_w
    slide_h = float(wsi_size[1]) if wsi_size else csv_wsi_h
    slide_bounds = _slide_bounds_fallback(gt_union, total_size, slide_w, slide_h)
    ...
    for roi_name, coords in roi_data.items():
        ...
        threshold, _, entry_debug = _get_hit_threshold(
            cache, case_name, _make_mag_key(mag), _make_size_key(roi_w, roi_h),
...
def main():
    ...
    parser.add_argument("result_folders", nargs="+",
                        help="One or more folders containing CPathAgent output JSON files")
    parser.add_argument("--gt-dir", nargs="+", default=DEFAULT_GT_DIRS,
                        help="One or more GT directories containing case CSVs or XMLs")
    ...
    parser.add_argument("--patch-size", type=int, default=224,
                        help="Stage-1 patch size in pixels (used to report screening coverage). Default: 224")
    parser.add_argument("--wsi-size", nargs=2, type=int, default=None,
                        metavar=("W", "H"),
                        help="WSI dimensions in 40x pixels (W H). Overrides the auto-computed "
                             "size derived from groups_all patch count. Only needed when the "
                             "auto estimate is too inaccurate.")
    parser.add_argument("--ids-bbox", default=None,
                        metavar="BBOX_DIR",
                        help="Directory of JSON files mapping patch_id -> [x1,y1,x2,y2] at "
                             "highest magnification. Prints every patch ID in all regions' "
                             "id_list with its corresponding bbox.")
    args = parser.parse_args()

    wsi_size = tuple(args.wsi_size) if args.wsi_size else None
    ...
    # Collect all CPathAgent output JSON files across all result folders
    json_paths = []
    for result_folder in args.result_folders:
        json_paths.extend(sorted(glob.glob(os.path.join(result_folder, "*.json"))))

    id_bbox_map: dict[int, list] | None = None
    if args.ids_bbox:
        id_bbox_map = load_id_bbox_map(args.ids_bbox)
        print(f"ID-bbox map loaded: {len(id_bbox_map)} entries from {args.ids_bbox}")
    ...
    for json_path in json_paths:
        res = evaluate_case(json_path, gt_index,
                            patch_size=args.patch_size,
                            wsi_size=wsi_size,
                            qa_index=qa_index,
                            id_bbox_map=id_bbox_map,
                            hit_threshold_cache=hit_threshold_cache)
        if res is None:
            try:
                with open(json_path) as f:
                    meta = json.load(f)
